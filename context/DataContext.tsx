...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    try {
      ...
      if (administrativosData) {
        console.log(
          ...
          JSON.parse(administrativosData).length
        );
        updates.push(
          new Promise((resolve) => {
        ...
        console.log(
          "No se encontraron datos de administrativos en AsyncStorage"
        );
      }
      ...
      if (actividadesData) {
        console.log(
          ...
          JSON.parse(actividadesData).length
        );
        updates.push(
          new Promise((resolve) => {
  ...
  useEffect(() => {
    if (docentes.length > 0) {
      ...
      docentes.forEach((docente) => {
        if (
          docente.materias &&
          Array.isArray(docente.materias) &&
          ...
          docente.materias.forEach((materia) => {
            ...
            const materiaExiste = materias.some(
              (m) => String(m.id) === String(materia.id)
            );
            const yaAgregada = materiasDeDocentes.some(
              (m) => String(m.id) === String(materia.id)
            );
      ...
      if (materiasDeDocentes.length > 0) {
        console.log(
          `Agregando ${materiasDeDocentes.length} materias desde docentes al sistema`
        );
    ...
    const saveAdministrativos = async () => {
      try {
        ...
        await AsyncStorage.setItem(
          "administrativos",
          JSON.stringify(administrativos)
        );
      } catch (e) {
  ...
  const addDirectivo = async (directivo: Omit<Directivo, "id">) => {
    try {
      const newDirectivo = {
        ...directivo,
        id: `directivo_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`,
      };
      ...
      await AsyncStorage.setItem(
        "directivos",
        JSON.stringify(updatedDirectivos)
      );
  ...
  const addAdministrativo = async (
    ...
    try {
      ...
      await AsyncStorage.setItem(
        "administrativos",
        JSON.stringify(updatedAdministrativos)
      );
      console.log(
        "Administrativo guardado en AsyncStorage:",
        newAdministrativo
      );
  ...
  const addActividad = (actividad: Omit<Actividad, "id">) => {
    const newActividad = {
      ...actividad,
      id: `actividad_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    };
  ...
  const updateDirectivo = async (
    id: string,
    updates: Partial<Omit<Directivo, "id">>
  ) => {
    try {
  ...
  const updateAdministrativo = async (
    id: string,
    updates: Partial<Omit<Administrativo, "id">>
  ) => {
    try {
  ...
  const updateActividad = (id: string, actividad: Partial<Actividad>) => {
    setActividades(
      actividades.map((a) => (a.id === id ? { ...a, ...actividad } : a))
    );
  };
      ...
      setEntregas((prev) => {
        ...
        if (existing) {
          ...
          return [
            ...prev,
            {
              id: `entrega_${Date.now()}_${Math.random()
                .toString(36)
                .substr(2, 9)}`,
              docenteId,
              materiaId,
  ...
  const clearMaterias = async () => {
    try {
      ...
      setMaterias([]);
      // Eliminar las materias de los docentes
      ...
      await AsyncStorage.setItem("materias", JSON.stringify([]));
      await AsyncStorage.setItem("docentes", JSON.stringify(updatedDocentes));

      return { success: true };
    } catch (error) {
      console.error("Error al limpiar materias:", error);
      return { success: false, error: "Error al limpiar materias" };
    }
  };
  ...
  const clearHorariosByEntity = (entityId: string, isDocente: boolean) => {
    ...
    console.log(
      `Eliminando todos los horarios para ${
        isDocente ? "docente" : "grupo"
      } ${entityIdStr}`
    );
    ...
    console.log(
      `Se eliminarán ${horarios.length - horariosRestantes.length} horarios`
    );
